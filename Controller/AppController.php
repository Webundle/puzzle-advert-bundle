...
class AppController extends Controller
{
    ...
    public function createPostulateAction(Request $request, $id)
    {
        ...
        if (! $postulate = $em->getRepository(Postulate::class)->findOneBy(['post' => $id, 'user' => $user->getId()])) {
            ...
            $this->addFlash('success', $this->get('translator')->trans('app.advert.postulate.create.success', ['%postName%' => $post->getName()], 'app'));
            return $this->redirectToRoute('app_advert_post_show', ['id' => $id]);
        }
        
        $this->addFlash('success', $this->get('translator')->trans('app.advert.postulate.create.duplicate', ['%postName%' => $post->getName()], 'app'));
        return $this->redirectToRoute('app_advert_post_show', ['id' => $id]);
    }
}
